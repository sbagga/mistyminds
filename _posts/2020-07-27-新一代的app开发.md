...
从App实现侧来看，随着组件化架构的采纳和组件生态的逐步繁荣，80%的app代码都可以使用现成组件实现，需要定制化的代码规模越来越少。组件包括UI组件，提供方主要是设计师，也包含特性组件，提供方是各种SaaS业务平台，SaaS除了提供云端API也越来越多地提供app级别的SDK和组件，达到即插即用的效果，即所谓的Feature as a Service，提供了认证、内容管理、地图、支付、搜索、音视频通信、电商、社交等App构建需要和的预制能力，也是OCX、ActiveX组件技术的自然演化。
...
剩下的一个关键问题是在UX设计师的工具生态和代码工程师的组件化自动化实现之间搭建一个互通的桥梁，这是实现UX自动打印出代码的关键。借用编译器的概念，我们需要定义设计和代码实现之间的UX IR（Intermediate Representation），这是标准化前端设计组件和app代码组件的关键性标准。再次借用编译器的语言，UX设计工具是这个编译器的frontend，这个编译器的backend是组件化的App框架，例如：React Native，快应用，Jetpack，UX IR是连接前端UX设计工具和后端App组件的桥梁，需要满足下面的要求

1. 无损描述UX设计工具的设计和交互输出
2. 提供UX设计工具组件模板和App代码模板之间的双向映射关系，包括名字、布局、style、字体的映射
3. UX组件的描述可以生成后端App组件的组合布局和数据graph视图
3. 通过UX IR可以生成高保真的UX原型
3. 通过UX IR可以生成不同后端App的基于组件的源代码包含组件布局，属性和数据graph关系等
