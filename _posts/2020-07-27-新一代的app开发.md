...
组件化的app开发理念已经在Uber, AirBnB, Twitter，Pinterest等互联网公司广泛使用，他们基于react等前端开发框架，构建了完整的组件开发、共享、生命周期管理等工具平台，统一了不同app之间的UX/UX设计语言，自动化了从设计团队到代码实现团队的组件的交接集成流程。这些理念也逐渐被开源实现，出现了繁荣的组件开发工具SaaS。随着组件化架构的采纳和组件生态的逐步繁荣，80%的app代码都可以使用现成组件实现，需要定制化的代码规模越来越少。

Framer，Storyboard，react-stylegUXdist – 可视化组件开发、管理平台
Bit – 组件的生命周期管理平台
...
设计驱动、组件化驱动的App设计自动化流程能够提高App开发的效率和质量，一流的互联网公司已经建立起与之配合的工具链和模板/组件生态系统，传统的Android Studio IDE已经不再是开发者的唯一的开发工具，这个状况非常类似云计算的发展，互联网公司因为业务的需求，自研了大量的大规模数据中心自动化配置和管理工具，这些技术后续才被以云计算的商业模式提供给企业和一般开发者，按照这个规律，在美国市场已经出现了越来越多的面向设计驱动的low coding原型设计平台，和面向组件化App开发的SaaS服务，这些SaaS服务把大公司的App开发实践兜售给企业和一般开发者。对第三市场的开发者，因为远离中美两个移动互联网创新高地，技术和商业的劣势导致本地的App开发的成本、创新的速度都处于落后状态，满足不了第三市场本地app生态减少的需求。
开源是迅速构建设计驱动和组件化的App开发的重要手段，是加速第三市场本地App开发，提高HMS普及和白牌App出海进入第三市场的重要技术手段。
