...
于此同时，web开始兴起，web的特点是极端的开放性，没有任何一家公司可以控制web生态和强迫开发者采纳某一种开发框架，导致web前端开发技术以一种无序状态在演化，从好的方面说，是百花齐放、天才辈出，从坏的方面说，web应用开发从来没有给开发者Delphi这样好用的开发工具和组件生态。
...
设计驱动的始作俑者是Apple，Apple的设计师是产品流程的owner，工程师团队只是他们设计打印机。这个流程被拷贝到移动应用行业作为样板开发流程，也极大影响了web app开发，赏心悦目的色彩，玄妙的动态效果，不需要思考的交互流程成为成功App的核心元素，也让App的开发过程更加复杂和昂贵。UX原型设计也已经出现了一个工具生态，允许UX设计师团队实时协作，和客户实时互动迭代设计，原型设计工具可以产生高保真的产品交互原型，运行在手机或者网页，设计驱动的极致目标就是把工程师团队这台打印机完全自动化，最终实现低代码（low coding）开发或者无代码（no coding）开发。
  ...
  - Invision，Figma，Sketch Cloud，蓝湖 – 可以实时交互协作的前端原型设计工具
...
从App实现侧来看，随着组件化架构的采纳和组件生态的逐步繁荣，80%的app代码都可以使用现成组件实现，需要定制化的代码规模越来越少。组件包括UI组件，提供方主要是设计师，也包含特性组件，提供方是各种SaaS业务平台，SaaS除了提供云端API也越来越多地提供app级别的SDK和组件，达到即插即用的效果，即所谓的Feature as a Service，提供了认证、内容管理、地图、支付、搜索、音视频通信、电商、社交等App构建需要和的预制能力，也是对20多年前OCX、ActiveX组建技术的重生。

![FaaS](../images/faas.png)


这个趋势是不是和云端的cloud native应用开发趋势是一致的？只是组件这个名字要换成micro serviceh或者容器，其实质是类似的，组件化、可组合的软件开发方式统一了前后台功能的开发的方法，需要定制的代码越来越少，而且主要集中在粘合逻辑。依照这个逻辑去对比前端组件开发和后端的微服务开发，我们可以看到前端技术还缺乏一些必要的工具链，来自动化前端组件驱动的开发方式。如下图所示，组件领域已经出现了开发工具，生命周期管理工具，组件库等开源项目，生态正在逐渐壮大。但是还缺乏一个标准化和系统化的前端组件工具链，
...
剩下的一个关键问题是在UX设计师的工具生态和代码工程师的组件化自动化实现之间搭建一个互通的桥梁，这是实现UX自动打印出代码的关键。借用编译器的概念，我们需要定义设计和代码实现之间的UX IR（Intermediate Representation），这是标准化前端设计组件和app代码组件的关键性标准，UX IR要解决的问题
1. 高保真描述设计工具的设计输出
2. 交互输出
3. 和代码部件的映射连接


主流的前端UX设计工具多数提供plugin机制，允许第三方开发者调用内部API和数据，设计项目也可以输出为格式化的文本，但是存在一些问题
设计工具在输出数据上没有统一的标准，虽然大多数工具都支持CSS的输出，但是
缺乏设计输出的统一标准，这个标准不仅仅是描述设计，为了和后端的代码组件生态对接，
缺乏标准导致生态碎片化，不同的设计工具

通过IR可以有效建立和前端设计工具生态的合作，使得UX的handoff过程输出的数据格式标准化，这样也极大提升了前端设计工具low coding的价值，同时UX IR对backend，即各种app的组件框架的自动化编译能力，编译生成的JSX、SwiftUI或者其他声明式UX程序脚X本，也即app组件代码的构建蓝图。UX IR 从而可以实现设计驱动全流程的自动化。

![UXIR](../images/UXIR.png)X


前端设计工具一般都支持通过plugin机制来读取UX设计的功能，这样视觉设计可以转变为UX IR描述，并且也实现了类似面向对象设计软件这样的可以从主设计模板继承开发设计模板的模板体系，对设计师的设计空间的约束是有限的。

围绕UX IR标准可以规范面向设计师的App UX 设计模板，这些模板都需要符合设计工具输出UX IR的一些需求，例如：命名，数据表征，元数据标注等，这些模板的基础元素可以基于一个符合UX IR的主模板生成，同时也需要把app代码组件尤其是FaaS组件映射回设计模板，这样设计师可以直接选取FaaS的设计组件做设计，而不需要凭空创造。

业界已经有一些SaaS服务和开源项目试图自动化从UX设计工具到代码生成的流程，例如AppChef项目，这个项目提供基于Sketch的设计模板和plugin，设计师基于模板的设计可以无缝映射到React Native的组件，设计师完成设计之后，可以通过plugin把设计上传到云端，云端会生成中间状态的设计描述schema文件，基于schema文件，可以渲染出基于真实组件实现的高保真UX App，允许设计师和客户迭代设计，同时也会生成react native app的源代码，在源代码基础上App工程师可以完成和后端数据服务基于GraphQL的数据集成和其他粘合逻辑的设计。整个App的开发流程是设计师驱动的，只有少量的代码需要在Android Studio IDE开发。AppChef的这个中间状态schema已经比较接近我们上述的UX IR，可以以这个项目为基础来开发更加完善的UX IR。
...
开源是迅速构建设计驱动和组件化的App开发的重要手段，是加速第三市场本地App开发，提高HMS生态普及和白牌App出海进入第三市场的重要技术手段。
 ...
 - UX IR建立的组件标准之后，可以建立HMS的组件库和驱动国产App在第三市场提供白牌App或者Feature as a Service的商业模式，在技术栈上叠加内容和商业，建立完备的App的供应链，全面加速第三市场App创新的速度。
